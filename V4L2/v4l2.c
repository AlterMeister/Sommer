...
static int v4l2_set_format(void)
{
    ...
    /*** 判断是否已经设置为我们要求的RGB565像素格式 如果没有设置成功表示该设备不支持RGB565像素格式 */
    // 可以改的，如果不是RGB565  就改需要的
    ...
    streamparm.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;  // 获取数据流
    ...
    for ( ; ; ) {
        ...
        for(buf.index = 0; buf.index < FRAMEBUFFER_COUNT; buf.index++) {
            ...
            for (j = 0, base=screen_base, start=buf_infos[buf.index].start;
                ...
                memcpy(base, start, min_w * 2);         // 将数据写入
                base += width;                          // LCD显示指向下一个地址
                start += frm_width;                     // 写入数据也往下指向一个地址
            }
